...
using namespace AthensRun;

namespace
{
// Trace styling: Garmin-ish chunky breadcrumb.
...
void MapTileView::draw(const Rect& area) const
{
    ...
    if (mContainer == nullptr || !mContainer->isOpen() || mCache == nullptr) {
        return;
    }
    ...
    const int64_t cx = MapMath::rescale(mCenterX16, 16, mZoom);
    const int64_t cy = MapMath::rescale(mCenterY16, 16, mZoom);
                            ...
                            int16_t absDX, int16_t absDY) const
{
    ...
    for (int64_t ty = top >> MapMath::TILE_SHIFT; ty <= (bottom >> MapMath::TILE_SHIFT); ++ty) {
        for (int64_t tx = left >> MapMath::TILE_SHIFT; tx <= (right >> MapMath::TILE_SHIFT); ++tx) {
            ...
            // source-relative blitRect -- the spike-proven recipe.
                            ...
                            int16_t absDX, int16_t absDY) const
{
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        const int32_t x = static_cast<int32_t>(
            MapMath::rescale(p.x, 16, mZoom) - viewOriginX);
        const int32_t y = static_cast<int32_t>(
            MapMath::rescale(p.y, 16, mZoom) - viewOriginY);
        if (i > 0) {

...
| `prototype/una_fts_walk.py` | **Working, validated code.** Read-only recursive FTS filesystem walk. Identified the `0x51` entry's trailing 12 bytes (mtime + size) and proved the `0x11` header's `uint16` size ceiling truncates silently; see §2.2, §2.2.1, §2.2.2. |

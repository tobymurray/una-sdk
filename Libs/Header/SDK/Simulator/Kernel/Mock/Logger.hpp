 ...
 ******************************************************************************
 *
 * Invariant to preserve: this header must not include SDK/UnaLogger/Logger.h and
 * must contain no LOG_* call. Keeping it that way means nothing in these inline
 * bodies depends on the including translation unit's LOG_LEVEL, so every TU
 * compiles identical definitions for them; and Kernel.hpp -- which every
 * simulator TU includes -- stays out of the set of headers that transitively pull
 * in Logger.h, so a .cpp can still set LOG_MODULE_PRX / LOG_MODULE_LEVEL after
 * its other includes.
 *
 ******************************************************************************
 */

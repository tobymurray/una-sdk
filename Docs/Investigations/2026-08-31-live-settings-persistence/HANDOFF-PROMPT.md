...
- **`save()` and `load()` sit in adjacent C++ vtable slots.** The vtable was found
  ...
  > **UPDATE 2026-08-31, this session:** this vtable base was one slot off — see
  > `README.md` in this folder. The real base is `0x081757b0`, with `load()` at
  > `+0x08` and `save()` at `+0x0c`. Keep this original entry for the historical
  > record of how the correction was reached, but use `README.md`'s corrected
  > layout for anything new.
- **A mutex-like lock guards this struct**: lock object at `0x20010ca0` (the
  ...
  > **UPDATE 2026-08-31, this session:** REFUTED as Settings-specific — see
  > `README.md`. It's a generic compiler-emitted guard-variable helper shared by
  > ~149 unrelated call sites kernel-wide.
...
- `heartRateZones`: 6 × `uint8_t` at `0x20010cb0 + 15` through `+20` (inclusive),
  ...
  > **UPDATE 2026-08-31:** corrected to `+0x10`..`+0x15` and CONFIRMED — see
  > `README.md`.
- `dailyGoals.activityMinutes` / `.steps` / `.floors`: read as 32-bit values at
  ...
  > **UPDATE 2026-08-31:** corrected to `+0x18`/`+0x1c`/`+0x20` and CONFIRMED —
  > see `README.md`.
- A `weight` field, read as a **float** (`vldr s0, [r4, #40]` or thereabouts, seen
  ...
  > **UPDATE 2026-08-31:** corrected to `+0x28` and CONFIRMED — see `README.md`.
...
- Whether `"Settings.Local"` (`local_settings.json`, fields `alertMute`,
  ...
  > **UPDATE 2026-08-31:** REFUTED (it's a sibling class with its own vtable, not
  > the same class) — see `README.md`.
...
> **UPDATE 2026-08-31:** Ghidra was already installed on this host (headless
> PyGhidra works) — Phase A used it directly. `jadx` (for the Android companion
> app) and `hbc-decompiler` (for its Hermes JS bundle) were added this session
> too — see `README.md`.
...
> **UPDATE 2026-08-31:** Phase A ran. Constructor found and confirmed
> (`0x080abbb4`, 3 real callers). But the vtable base itself was corrected (see
> §4 update above), and — critically — **no real call site for `save()` was
> found**: zero direct `bl` hits, zero indirect-vtable-dispatch hits through the
> corrected save slot (`+0x0c`) near any of ~3700 `blx` sites checked. `load()`
> does have 2 confirmed real callers. Phase B is therefore blocked — see
> `README.md` for the full ledger and the new candidate approach (BLE FTS write,
> via the already-proven `investigate/ble-large-file-transfer` work) that doesn't
> depend on finding save()'s caller at all.
...
> **UPDATE 2026-08-31:** tried the falsification path directly (the string xref)
> since Phase A/B stalled. The only real caller of that string comparison is a
> file-transfer-FSM state handler that looks like a disguised factory-reset
> trigger operating on a local staging struct, not a generic JSON writer — see
> `README.md`. Also a dead end for the originally-planned route.

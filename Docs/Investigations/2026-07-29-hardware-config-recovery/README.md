...
~5s, reconnect, read the file back. `REPRODUCTION-GUIDE.md`, alongside this file, is the
verified build and deploy workflow, command by command.
...
| Exact part = STM32U5A5 (not U5A9) | **CONFIRMED** | Three independent methods agree: (1) a physical teardown in a prior session read the chip marking off the die as STM32U5A5; (2) FLASH_OPTR word2 (0x0BF9007F) decodes to boot address `0x0BF90000`, confirmed by multiple sources as the STM32U5 system-bootloader ROM address, with one source naming STM32U5A5 explicitly; (3) the LTDC-all-zero read matches exactly what the real ST datasheet (DS13543, downloaded and parsed locally) predicts for U5A5 parts — **LTDC = No** for every U5A5 SKU, **LTDC = Yes** only for U5A9/599/5Fx/5Gx. All three independent signals converge; this closes the MCU-identity question. |
